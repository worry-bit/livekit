...
- 外屏双路直播时，`foldedDualStreamPanel()` 会同时保留淘宝和抖音两个 `XComponent` surface，用层级切换当前可见直播，避免切换时销毁另一直播画面。
...
- 外屏双路都处于直播态时，左上角显示当前直播状态，右上角显示绿色“切换直播”按钮。点击只改外屏可见平台，不改变展开屏左淘宝、右抖音布局。
